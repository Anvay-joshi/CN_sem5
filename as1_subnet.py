...
def main():
    ...
    subnet_mask_updated = subnet_mask 
    for i in range(borrowed_bits):
        subnet_mask_updated += (1 << (32 - min(valid_subnet_masks[subnet_class]) - i - 1))
    print(f"subnet mask is: {int_to_address(subnet_mask_updated)}")
